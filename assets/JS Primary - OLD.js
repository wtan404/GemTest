  ...
  constructor() {
    ...
    this.productTitleItems.forEach((titleItem) => {
      titleItem.addEventListener('mouseenter', () => {
        this.labelDisplay.textContent = titleItem.textContent;
      });

      titleItem.addEventListener('mouseleave', () => {
        this.labelDisplay.textContent = originalText;
      });
    });
  }
}

if (!customElements.get('product-sibling')) {
  customElements.define('product-sibling', ProductSibling);
}
/**
...
if (!window.theme.shapesAnimationsInitialized) {
  ...
  class ShapesAnimations {
    ...
    initParallax() {
      ...
      this.intensity =
        map(window.theme.settings.parallaxIntensity, 0, 100, 1, 110) / 100;
  ...
  parseDesignProperties(design) {
    ...
    Object.keys(customProps).forEach(key => {
      if (key.startsWith('customization-')) {
        // Convert customization-color -> _blankCustom_color
        const shopifyKey = '_' + key.replace(/-/g, '_');
        properties[shopifyKey] = customProps[key];
      } else if (key.startsWith('printable-area-')) {
        // Convert printable-area-top -> _blankCustom_printable_area_top
        const shopifyKey = '_blankCustom_' + key.replace(/-/g, '_');
        properties[shopifyKey] = customProps[key];
      }
    });
    ...
    if (customProps['bb-design']) {
      try {
        ...
        if (legacyDesign.color && !properties['_blankCustom_color']) {
          properties['_blankCustom_color'] = legacyDesign.color;
        }
        if (legacyDesign.logoUrl && !properties['_blankCustom_logo_url']) {
          properties['_blankCustom_logo_url'] = legacyDesign.logoUrl;
        }
        if (legacyDesign.logoPos && !properties['_blankCustom_logo_position']) {
          properties['_blankCustom_logo_position'] = legacyDesign.logoPos;
        }
        if (legacyDesign.logoSize && !properties['_blankCustom_logo_size']) {
          properties['_blankCustom_logo_size'] = legacyDesign.logoSize;
        }
        if (legacyDesign.colorName && !properties['_blankCustom_color_name']) {
          properties['_blankCustom_color_name'] = legacyDesign.colorName;
        }
        if (legacyDesign.bottleImage && !properties['_blankCustom_base_product_image_url']) {
          properties['_blankCustom_base_product_image_url'] = legacyDesign.bottleImage;
        }
        
        // Map printable area from legacy format
        if (legacyDesign.printable) {
          if (legacyDesign.printable.top && !properties['_blankCustom_printable_area_top']) {
            properties['_blankCustom_printable_area_top'] = legacyDesign.printable.top;
          }
          if (legacyDesign.printable.bottom && !properties['_blankCustom_printable_area_bottom']) {
            properties['_blankCustom_printable_area_bottom'] = legacyDesign.printable.bottom;
          }
          if (legacyDesign.printable.left && !properties['_blankCustom_printable_area_left']) {
            properties['_blankCustom_printable_area_left'] = legacyDesign.printable.left;
          }
          if (legacyDesign.printable.right && !properties['_blankCustom_printable_area_right']) {
            properties['_blankCustom_printable_area_right'] = legacyDesign.printable.right;
          }
        }
      } catch (e) {
        console.warn('Failed to parse legacy bb-design data:', e);
    ...
    if (!properties['_blankCustom_acknowledged']) {
      properties['_blankCustom_acknowledged'] = 'true';
    }
    return properties;
  }
